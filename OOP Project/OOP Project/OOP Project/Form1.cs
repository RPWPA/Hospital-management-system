    ...
    public partial class Form1 : Form
    {
                ...
                foreach (KeyValuePair<string, Nurse> search in nurse)
                {
                    if (text_delete_nurse_id.Text == search.Key)
                    {
                        ...
                        if (search.Value.get_nurs_patients() != null)
                        {
                            ...
                            for (int i = 0; i < 5; i++)
                            {
                                if (N[i] != text_doc_ID_delete.Text)
                                {
                                    for (int j = 0; j < count; j++)
                                    {
        ...
        private void edit_save_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (pat.Count() != 0)
                {
                    foreach (var bc in pat)
                    {// deleting patients inside the list.
                        if (bc.Value.get_department() == edit_combobox.Text)
                        {
                            pat.Remove(bc.Value.get_id());
                        }
                    }
                }
                if (doc.Count() != 0)
                {
                    foreach (var bc in doc)
                    {// deleting doctors inside it.
                        if (bc.Value.get_department() == edit_combobox.Text)
                        {
                            doc.Remove(bc.Value.get_id());
                        }
                    }
                }
                if (dplist.Count() != 0)
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            {// deleting the patients inside of it.
                if (bc.Value.get_department() == delete_combobox.Text)
                {
                    pat.Remove(bc.Value.get_id());
                }
            }
            foreach (var bc in doc)
            {// deleting the doctors inside of it.
                if (bc.Value.get_department() == delete_combobox.Text)
                {
                    doc.Remove(bc.Value.get_id());
                }
        ...
        private void done_button_to_add_appointment_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (int index = 0; index < view_doctor_list.SelectedItems.Count; index++)
                    {
                        appo.Add(new Appointment(date1, Patient_ID_TxtBox.Text, view_doctor_list.SelectedItems[index].SubItems[0].Text));
                    }
                    ok = true;
                        min_textbox.Text = " ";
                    hours_textbox.Text = " ";
        ...
        private void delete_app_button_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                foreach (Appointment search in appo)
                {
                    if (get_hours_todel.Text == search.Datetime.Hour.ToString() && get_min_todel.Text == search.Datetime.Minute.ToString() && get_day_todel.Text == search.Datetime.Day.ToString()
                                  && get_month_todel.Text == search.Datetime.Month.ToString() && get_year_todel.Text == search.Datetime.Year.ToString())
                    {
                        for (int i = 0; i < appo.Count; i++)
                        {
                            if (get_pateint_id_todel.Text == appo[i].get_id_pateint())
                            {
        ...
        private void Next_Patient_btn_Click(object sender, EventArgs e)
        {
            ...
            if (done && id_done)
            {
                if(Resident_radBtn.Checked == true)
                {
                    ...
                    Patient p = new Patient(Patient_Name_TxtBox.Text, Patient_Age_TxtBox.Text, Patient_ID_TxtBox.Text, Patient_Phone_TxtBox.Text, combobox_patient_department.Text, patiet_history.Text, comboBox_select_room.Text);
                ...
                else if (Appointment_radBtn.Checked == true)
                {
                    ...
                    {
                        string q = "";
                        Patient p = new Patient(Patient_Name_TxtBox.Text, Patient_Age_TxtBox.Text, Patient_ID_TxtBox.Text, Patient_Phone_TxtBox.Text, combobox_patient_department.Text, patiet_history.Text, "");
                        for (int index = 0; index < view_doctor_list.SelectedItems.Count; index++)
                        {
                           q = view_doctor_list.SelectedItems[index].SubItems[0].Text;
                        }
                        p.Add_Doctor(q);
